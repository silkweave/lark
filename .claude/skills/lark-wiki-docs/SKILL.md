---
name: lark-wiki-docs
description: Manage Lark/Feishu resources via MCP — Wiki documents, Base (Bitable) tables, messaging, and contacts. Use when the user asks to set up @silkweave/lark, authenticate with Lark, create/update Wiki documents, manage Base tables/records, send messages, or list contacts. Also acts as a setup wizard for first-time users.
---

# Silkweave Lark MCP Skill

Manage Lark/Feishu resources through MCP tools exposed by the `@silkweave/lark` package.

## Setup Wizard

If the user hasn't set up @silkweave/lark yet, guide them through these steps:
...
**Other MCP clients** -- use the same command: `npx -y @silkweave/lark mcp`
