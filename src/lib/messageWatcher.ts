...
export class MessageWatcher implements GatewayHost {
  ...
  private async handleMessage(data: MessageEvent) {
    try {
      ...
      // A reply in a thread that a mention started is addressed to the bot without a re-mention — the
      // `mentionBot` subscription gate must honor it, or thread follow-ups never match (and thus never
      // reach reflex/dispatch at all).
      const addressedToBot = mentionedBot || this.isEngagedThread(message)
      ...
      const matched = config.subscriptions.filter((s) => this.matches(s, message.chat_id, addressedToBot, text))
      ...
      const mentionEngaged = addressedToBot
  ...
  private matches(subscription: MessageSubscription, chatId: string, addressedToBot: boolean, text: string): boolean {
    if (subscription.chatId && subscription.chatId !== chatId) { return false }
    if (subscription.mentionBot && !addressedToBot) { return false }
